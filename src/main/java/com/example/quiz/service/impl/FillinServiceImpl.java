...
public class FillinServiceImpl implements FillinService {
		...
		try {
			...
			for (Question item : quList) {
				...
				for (Fillin fillin : fillinList) {
					...
					// 檢查選項與答案
					checkResult = checkOpionAnswer(item, fillin);
					if(checkResult != null) {
						return checkResult;
					}
				}
			...
			fillinStr = mapper.writeValueAsString(req.getFillinList());
	...
	private BasicRes checkOpionAnswer(Question item, Fillin fillin) {
		// 1. 檢查必填也要有答案
		// fillin 中的答案沒有值，返回錯誤
