...
//			quizDao.save(quiz);
			...
			// 若存在 --> 更新；不存在 --> 新增
			// req 中沒有在該欄位時，預設是 0 ，因為 id 的資料型態是 int
	...
	private BasicRes checkParams(CreateOrUpdateReq req) {
		...
		// req.getStartDate().isAfter(LocalDate.now()): 若 req 中的開始時間比前時間"晚"，會得到 true
	...
	public BasicRes delete(DeleteReq req) {
		...
		if (!CollectionUtils.isEmpty(req.getIdList())) {
			...
			try {
				...
				// 當 deleteAllById 方法中，id 的值不存在時，JPA 會抱錯
				// 因為在刪除之前，JPA 會先搜尋帶入的 id 值，若沒結果就會報錯
				// 實際上也沒刪除任何資料，所以就不需要對這個 Exception 做處理
			}
		}
		return new BasicRes(ResMessage.SUCCESS.getCode(), //
				ResMessage.SUCCESS.getMessage());
